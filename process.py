...
def main(args):
    ...
    print(f"Processing {len(experiment_data)} experiments with {args.n_workers} workers...")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ref_genome", type=str, help="Reference genome file", default="encode_data/genomes/GRCh38.fa"
    )
    parser.add_argument(
        "--annotation", type=str, help="Annotation file", default="encode_data/genomes/GRCh38_v24.gtf"
    )
    parser.add_argument(
        "--db", type=str, help="Annotation database file", default="encode_data/genomes/GRCh38_v24_annotation.sqlite"
    )
    parser.add_argument(
        "--chrom_sizes", type=str, help="Chromosome sizes file", default="encode_data/genomes/GRCh38.chrom.sizes.tsv"
    )
    parser.add_argument(
        "--ensembl_to_uniprot", type=str, help="Ensembl to UniProt mapping file", default="encode_data/ensembl_to_uniprot.pkl"
    )
    parser.add_argument(
        "--assay", type=str, help="Assay type (ATAC-seq/CAGE)", default="ATAC-seq"
    )
    parser.add_argument(
        "--n_workers", type=int, help="Number of workers", default=None
    )
